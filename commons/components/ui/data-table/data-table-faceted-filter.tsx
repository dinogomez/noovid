...
export function DataTableFacetedFilter<TData, TValue>({
  ...
  React.useEffect(() => {
    if (
      defaultValueSelected &&
      options.some((option) => option.value === defaultValueSelected)
    ) {
      const filterValues = [defaultValueSelected];
      column?.setFilterValue(filterValues);
    }
  }, [column, defaultValueSelected, options]);
  return (
    <Popover>
                  ...
                  options
                    ...
                    .map((option) => (
                      <Badge
                        variant="secondary"
                        key={option.value}
                        className="rounded-sm px-6 font-normal w-fit"
                      >
                        {option.label}
                      </Badge>
                    ))
